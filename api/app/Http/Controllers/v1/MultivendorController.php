...
class MultivendorController extends Controller
{
    ...
    public function addRemoveProduct(Request $request)
    {
        ...
        //print_r($product);die;
        $vendor_id = $product['department_id'];

        $vendor = Departments::find($vendor_id);
        ...
        $bundle = CustomerBundle::with('products')->where('vendor_id',$vendor_id)->find($bundle_id);
    ...
    public function logs(Request $request)
    {
        ...
        $messages = [
            'action.required' => 'Action is required.',
            'action.in' => 'Action must be one of add, subract or remove.',
            'vendor_id.required' => 'Please provider Product Id'
        ];
        ...
        if (is_null($vendor)) {
            ...
            return response()->json([
                ...
                'errors' => [
                    'product_id' => [
                        "The vendor ID seems to be invalid."
                    ]
                ],
            ], 422);
        ...
        $log = CustomerLogs::with('vendors')->find($log_id);
        ...
        if (!$log) {
            ...
            $log = CustomerLogs::create([
                'customer_id' => $user ? $user->id : null,
                'vendor_id' => $vendor_id,               
            ]);


        } else if($log && is_null($log->customer_id) && $user) {
        ...
        foreach ($log->departments as $v ) {
           
            if ($v['vendor']->id == $vendor->id) {
        ...
        if ($in_log && $action == "add") {

            $attributes = [
                'quantity' => $in_log['quantity'] + 1,
            ];

            $log->vendors()->updateExistingPivot($vendor->id, $attributes);

        }  else if(!$in_log && $action === "add") {

            $log->vendors()->attach($vendor->id);
    ...
    {   
        ...
        $carts = DB::table("customer_bundles")->select("id","name","vendor_id")->where('status', '=' , 'active')->whereIn('id', $cartIds)->get();
    ...
    //get all vendor cart info
    public function showGlobalCart(Request $request)
    {
        ...
        $collection = new CustomerBundleCollection(CustomerBundle::whereIn('id', $cartIds)->where('status', '=' , 'active')->paginate($per_page));
