...
class Bundles extends Model
{
    ...
    public function getPriceAttribute()
    {
        ...
        foreach ($products as $product) {
            $price = $price + ($product->maximum_selling_price * $product->pivot->quantity);
        }

        $this->price = (float) round($price, 2);

        if($this->maximum_selling_price){

            $this->price = $this->maximum_selling_price;
...
//////////////// we have to check this code////////////////////
    ...
    public function toArray()
    {
        ...
        return [
            ...
            'category' => Hashids::encode($this->category_id),
            'name' => $this->name,
