                    ...
                    <div class="col-md-8">
                      <div class="table-responsive">
                        <table class="table table-bordered">
                          <thead>
                            <tr>
                            <th colspan="4">PRO Items<span style="float: right;"><button class="btn btn-info" type="button" id="add_item">ADD ITEM</button></span>
<input type="hidden" name="bundle_id" id="bundle_id" value="<?php echo $id; ?>">
      ...
      <div class="modal-dialog">
        ...
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-info" id="add_prod" data-dismiss="modal">SUBMIT</button>
    ...
    <script type="text/javascript">
      ...
      $("#add_prod").on('click', function(){
        ...
        if(bundle_item.length > 0){
          var ids = JSON.stringify(bundle_item);
           ...
           $.ajax({
            type: "POST",
            url: '<?php echo base_url('Coupons/get_products'); ?>',
            data: 'ids='+ids,
      ...
      function bundle_add_list(items){
        ...
        prod_list_bundle += '<tr id="item_'+element.id+'"><td>'+element.name+'</td><td>'+element.valucart_price+'</td><td><input type="number" name="qty[]" value="1" min="1" max="10" onblur="qty_change(this,'+element.id+')" /></td><td><button class="remove" data-id="'+element.id+'">&times;</button></td><input type="hidden" value="'+element.id+'" name="ids[]" /></tr>';
    ...
    function click_process() {
      ...
      $("#save").on('click', function(){
        ...
        var post_data = {"items": items, "bundle_id": bundle_id, "bundle_price": bundle_price};
        console.log(parseInt(bundle_price));
        if(bundle_price <= 0 ){
          ...
          $.ajax({
            type: "POST",
            url: '<?php echo base_url('Bundles/save_new_bundle'); ?>',
            data: post_data,
            success: function(response){
              if(response == 1){
                window.location.href = '<?php echo base_url('Bundles/add_alternate/'); ?>'+bundle_id
              }
            }
          });        
        }
