...
class Marketing_model extends CI_Model {
    ...
    function get_marketing(){
        ...
        $query = $this->db->get('marketingteam');
        $result = $query->result();
        return $result;
    }


    function save_marketing($data,$object_id) {
        
        $this->db->where('email', $data['email']);
        $this->db->or_where('phone_no',$data['phone_no']);
        $this->db->from('marketingteam');
        ...
        if($count > 0 || $count_user >  0) {
            ...
            $this->db->insert('marketingteam', $market_data); 
            ...
            $log = array(
                         'id' =>$insert_id,
                         'log' => 'Created Marketing Member '.$data['first_name']. ''
                      );
    ...
    function get_single_marketing($id) {
    ...
    function update_marketing($data, $id,$object_id) {
        $this->db->where("id !=",$id);
        ...
        if($count > 0) {
            ...
            $this->db->where('id',$id);
            $this->db->update('marketingteam', $tech_data); 
            $where = array('company_id' => '1',
                            'user_id' => $id,
                            'user_type_id' => '3');
            ...
            $this->db->where($where)->update('users', $rs); 
                ...
                $log = array(
                         'id' =>$id,
                         'log' => 'Updated Marketing Member '.$data['first_name']. ''
                      );
